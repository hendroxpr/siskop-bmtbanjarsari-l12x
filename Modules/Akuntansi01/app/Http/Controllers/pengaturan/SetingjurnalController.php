...
namespace Modules\Akuntansi01\Http\Controllers\pengaturan;
use DateTime;
use Carbon\Carbon;
...
class SetingjurnalController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $remark = 'Halaman ini digunakan untuk menampilkan, menambah, mengubah dan menghapus <b>Seting Jurnal</b>.';
        $page = 'akuntansi01::pengaturan.setingjurnal';
        $link = '/akuntansi01/pengaturan/setingjurnal';
        $subtitle = 'Pengaturan';
        $caption = 'Seting Jurnal';
        $jmlhal = 2;
        ...
        return view($page, [
            ...
            'remark' => $remark,
            'jmlhal' => $jmlhal,
            'tabel' => Setingjurnal::SimplePaginate($jmlhal)->withQueryString(),
    ...
    public function create(Request $request)
    {
        ...
        if ($id == '0') {
            Setingjurnal::create($data);
        } else {
            Setingjurnal::where('id', '=', $id)->update($data);
        }
    ...
    public function show()
    {
        ...
        $data = $datax
            ...
            ->addColumn('action', function ($row) {
                return '<a href="#" title="Edit Data" class="btn btn-success btn-xs item_edit" data="' . $row->id . '" data2="'. $row->kode.'" data3="'. $row->jurnal.'" data4="'. $row->jurnal.'"><i style="font-size:18px" class="fa">&#xf044;</i></a> ' .
                       '<a href="#" title="Hapus Data" class="btn btn-danger btn-xs item_hapus" data="' . $row->id . '" data2="'. $row->kode.'" data3="'. $row->jurnal.'" data4="'. $row->jurnal.'"><i style="font-size:18px" class="fa">&#xf00d;</i></a>';
            })
    ...
    {
        $data = Setingjurnal::where('id', '=', $id)->get();
    ...
    {
        $data = Setingjurnal::where('id', '=', $id)->delete();
        return json_encode(array('data' => $data));
    }
