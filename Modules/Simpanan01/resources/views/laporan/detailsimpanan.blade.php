...
@php
    ...
    $tglawalx1 = session()->get('$tglawalx1');
    if($tglawalx1==''){
        $tglawalx1 = session()->get('memtanggal');
    }
    $tglakhirx1 = session()->get('$tglakhirx1');
    if($tglakhirx1==''){
        $tglakhirx1 = session()->get('memtanggal');
    }
        ...
        <div class="row">
            ...
            <div class="col-md-5">
                ...
                <div class="row">
                    ...
                    <div class="col-md-7">
                        <input name="jenissimpananx1" id="jenissimpananx1" class="w3-input w3-border" type="text" placeholder="Jenis Simpanan" value="{{ session('jenissimpananx1') }}" readonly>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-4 mt-2 text-right">
                    ...
                    <div class="col-md-3">
                        <input name="tglawalx1" id="tglawalx1" class="w3-input w3-border" type="text" placeholder="tgl awal" value="{{ session('tglawalx1') }}">
                    </div>
                    <div class="col-md-1 mt-2 text-center px-0">
                        <h6>s/d</h6>                         
                    </div>
                    <div class="col-md-3">
                        <input name="tglakhirx1" id="tglakhirx1" class="w3-input w3-border" type="text" placeholder="tgl akhir" value="{{ session('tglakhirx1') }}">
                    </div>
                </div>
                ...
                <table id="data1" class="table table-bordered table-striped table-hover" style="width: 100%">
                    <thead>
                        <tr>
							...
							<th style="width:50px">Tgl. Transaksi</th>
                        ...
                        <div class="modal-body" id="bodyAdd" name="bodyAdd">                        
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8 input-group">
                                    <input name="nomorbukti1" id="nomorbukti1" class="form-control w3-input w3-border rounded-0" type="search" placeholder="TRK.001.20230131.001" disabled value="{{ session('nomorbukti1') }}">                        
                                    <div class="input-group-append">
...
<div class="modal fade" id="ModalPosting"  data-backdrop="static">
    <div class="modal-dialog modal-default">  <!-- modal-(sm, lg, xl) ukuran lebar modal -->
        <div class="modal-content bg-warning w3-animate-zoom">
            ...
            <div class="modal-header">
                    <h3 class="modal-title"><i style="font-size:18" class="fa">&#xf093;</i><b> Posting tarik Simpanan</b></h3>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
...
$(document).ready(function(){
    ...
    function terbilang(bilangan) {
     ...
     function tampil_data1(){
        ...
        return $('#data1').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax   : `{{route('simpanan01.laporan.detailsimpanan_show')}}`,
            columns: [
                ...
                { data: 'norek', name: 'target.norek' },
                { data: 'nia', name: 'target.nia' },
                { data: 'nama', name: 'target.nama' },
                { data: 'jenissimpanan', name: 'jenissimpanan.jenissimpanan' },
                { data: 'sandi', name: 'sandi.kode', class: 'text-center' },
     ...
     function tampil_data_carinasabah(){
        ...
        return $('#carinasabah').DataTable({
            ...
            processing: true,  
            serverSide: true,
            ajax   : `{{route('simpanan01.laporan.detailsimpanan_shownasabah')}}`,
            columns: [
                // { data: 'no', name:'id', render: function (data, type, row, meta) {
                //     return meta.row + meta.settings._iDisplayStart + 1;
                // }},
    ...
    function cariid(cari){
        ...
        $.ajax({
            ...
            type   : 'post',
            url    : '{{route('simpanan01.laporan.detailsimpanan_cariid')}}',
            async : false,
            dataType : 'json',
            // data : FormData,
            data : {cari1:cari1},
    ...
    function carinomorbukti(cari){
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('simpanan01.laporan.detailsimpanan_carinomorbukti')}}',
            async : false,
            dataType : 'json',
            // data : FormData,
            data : {cari1:cari1},
    ...
    function ambilcari(t){
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                data1Datatable = tampil_data1();
                setTimeout(() => {
                    data1Datatable.ajax.url('{{route('simpanan01.laporan.detailsimpanan_show')}}').load();                
    ...
    function kirimsyarat(){
        ...
        var tglawalx1=$('#tglawalx1').val();
        var tglakhirx1=$('#tglakhirx1').val();
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('simpanan01.laporan.detailsimpanan_kirimsyarat')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
     ...
     $("#tglawalx1").datepicker({
           dateFormat  : "yy-mm-dd",
           changeMonth : true,
           changeYear  : true         
    });
    $("#tglakhirx1").datepicker({
           dateFormat  : "yy-mm-dd",
           changeMonth : true,
           changeYear  : true         
    });

    $('#tglawalx1').on('click',function(){
        kirimsyarat();
        setTimeout(() => {
            data1Datatable = tampil_data1();  
            setTimeout(() => {
    ...
    $('#tglakhirx1').on('click',function(){
        kirimsyarat();
        setTimeout(() => {
            data1Datatable = tampil_data1();  
            setTimeout(() => {
                data1Datatable.ajax.url('{{route('pinjaman01.laporan.rekappinjaman_show')}}').load();                
                data1Datatable.draw(null, false);
            }, 500);
        }, 500);
    });
    $('#tglakhirx1').on('change',function(){
        kirimsyarat();
        setTimeout(() => {
            data1Datatable = tampil_data1();  
            setTimeout(() => {
    ...
    function data_simpan(){
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('simpanan01.laporan.detailsimpanan_create')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function nomorbukti_ok(){        
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('simpanan01.laporan.detailsimpanan_nomorbukti')}}',
            async : false,
            dataType : 'json',
            // data : FormData,
    ...
    function nomorposting(){        
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('simpanan01.laporan.detailsimpanan_nomorposting')}}',
            async : false,
            dataType : 'json',
            // data : FormData,
    ...
    function data_edit(idx){
        var id1=idx;			
            ...
            $.ajax({
                type  : 'get',
                url   : `{{ url('simpanan01/laporan/detailsimpananedit')}}/${id1}`,
                async : false,
                dataType : 'json',	
    ...
    function data_posting(){
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('simpanan01.laporan.detailsimpanan_posting')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function data_hapus(){			
        ...
        $.ajax({
            type  : 'get',
            url   : '{{url('simpanan01/laporan/detailsimpanandestroy')}}/'+id3,
            async : false,
            dataType : 'json',					
            success : function(data){
                tampil_datatable();
    ...
    $("#btn_print6").on('click',function(){ 
        ...
        window.open('{{ url('simpanan01/laporan/detailsimpananprintdetail') }}/'+ idx);
    });
