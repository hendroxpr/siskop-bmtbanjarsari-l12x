                ...
                <div class="form-group">
                    <form class="form-horizontal" id="formSimpan" nama="formSimpan" action="" method="">
                        ...
                        <div class="modal-body" id="bodyAdd" name="bodyAdd">                        
                            ...
                            <div class="row">
                                <div class="col-md-4 mt-1" align="right">									
                                    <h6 class="mt-2">Jenis Pendapatan</h6>
                                </div>
                                <div class="col-md-8">
                                    <select name="idkategoribiaya1" id="idkategoribiaya1" class="w3-input w3-border"></select>
                                </div>								  
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-4 mt-1" align="right">									
                                    <h6 class="mt-2">Item Pendapatan</h6>
                                </div>
                                <div class="col-md-8">
                                    <input name="pendapatan1" id="pendapatan1" class="w3-input w3-border" maxlength="100" type="search" placeholder="item  pendapatan" value="{{ old('pendapatan1') }}" required>
                                </div>								  
                            </div>
                            <div class="row">
                                <div class="col-md-4 mt-1" align="right">									
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">   
                                    <input name="hjs1" id="hjs1" class="w3-input w3-border text-right" type="search" placeholder="Harga Beli Satuan" value="0"  onkeydown="return numbersonly(this, event);" onkeyup="javascript:tandaPemisahTitik(this);">                             
                                </div>								  
                            </div>
                            <div class="row">
                                <div class="col-md-4 mt-1" align="right">									
                                    <h6 class="mt-2">Sub Total</h6>
                                </div>
                                <div class="col-md-8">                                
                                    <input name="subtotal1" id="subtotal1" class="w3-input w3-border" type="search" placeholder="sub total" style="text-align: right" value="0" readonly>
                                </div>								  
                            </div>
                            <div class="row">
                                <div class="col-md-4 mt-1" align="right">									
                            ...
                            <div class="row">
                                <div class="col-md-4 mt-1" align="right">									
                                    <h6 class="mt-2">Total Jual</h6>
                                </div>
                                <div class="col-md-8">                                
                                    <input name="totaljual1" id="totaljual1" class="w3-input w3-border" type="search" placeholder="Jumlah" style="text-align: right" value="0" readonly>
                                </div>								  
                            </div>
                            <div class="row">
                                <div class="col-md-4 mt-1" align="right">									
...
$(document).ready(function(){
    ...
    $('#idkategoribiayax1').on('change',function(){
        setTimeout(() => {
            $('#idkategoribiaya1').val($('#idkategoribiayax1').val());
            kirimsyarat();	
        }, 500);
    });
    ...
    $("#qty1").on('change',function(){ 
        ...
        if(j2=='0'||k2=='0'){
            ...
            $("#subtotal1").val(formatAngka(j3*k3,''));

        }
            var subtotalx = $("#subtotal1").val();

        if(j1==''||j2=='0'||k1==''||k2=='0'||subtotalx=='0'){
            document.getElementById("btn_simpan").disabled = true;
        }else{
            document.getElementById("btn_simpan").disabled = false;
        }

        var w1 = $("#subtotal1").val().replace(/[^,\d]/g, '').toString(); 
        var x1 = $("#ppn1").val().replace(/[^,\d]/g, '').toString(); 
        var y1 = $("#diskon1").val().replace(/[^,\d]/g, '').toString(); 

        var tb3 = parseFloat(w1) + parseFloat(x1) - parseFloat(y1);
        $("#totaljual1").val(formatAngka(tb3,''));
    ...
    function kirimsyarat(){
        ...
        $.ajax({
            ...
            type   : 'post',
            url    : '{{route('pos01.transaksi.pendapatanlain_kirimsyarat')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function tampil_listruang(){				
        $.ajax({
            type: 'get',
            url   : '{{route('pos01.transaksi.pendapatanlain_listruang')}}',
    ...
    function tampil_listkategoribiaya(){				
        $.ajax({
            type: 'get',
            url   : '{{route('pos01.transaksi.pendapatanlain_listkategoribiaya')}}',
            
            success: function(data){				    
                $("#idkategoribiayax1").html(data);
                $("#idkategoribiaya1").html(data);
                $("#idkategoribiayax1").val({{ $idkategoribiaya }});
                $("#idkategoribiaya1").val({{ $idkategoribiaya }});
    ...
    function tampil_listsatuan(){				
        $.ajax({
            type: 'get',
            url   : '{{route('pos01.transaksi.pendapatanlain_listsatuan')}}',
...
footerCallback: function (row, data, start, end, display) {
            ...
            api.column(9).footer().innerHTML = formatAngka(pagetotaljual,'');            
            },

            processing: true,
            serverSide: true,
            ajax   : `{{route('pos01.transaksi.pendapatanlain_show')}}`,
            columns: [
    ...
    function data_simpan(){
        ...
        let formData = new FormData();
            ...
            formData.append('idkategoribiaya1', idkategoribiaya1);
            formData.append('idsatuan1', idsatuan1);
            formData.append('nomorbukti1', nomorbukti1);
            formData.append('pendapatan1', pendapatan1);
            ...
            formData.append('totaljual1', totaljual1);
            formData.append('keterangan1', keterangan1);
          
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('pos01.transaksi.pendapatanlain_create')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
            ...
            error : function(formData){                    
                swalgagaltambah($('#pendapatan1').val());                 
                }
        });
    ...
    function data_edit(idx){
        ...
        var id1=idx;			
            $.ajax({
		        type  : 'get',
		        url   : `{{ url('pos01/transaksi/pendapatanlainedit')}}/${id1}`,
		        async : false,
		        dataType : 'json',	
    ...
    function data_hapus(){			
        ...
        $.ajax({
            type  : 'get',
            url   : '{{url('pos01/transaksi/pendapatanlaindestroy')}}/'+id3,
            async : false,
            dataType : 'json',					
            success : function(data){
                tampil_dataTable();
