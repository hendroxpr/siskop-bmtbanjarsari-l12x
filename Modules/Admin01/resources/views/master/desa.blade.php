...
@php
    ...
    $idkecamatan = session('idkecamatanx1');    
    $idkecamatan1 = session('idkecamatan1');    
@endphp
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="row mt-1">
                    ...
                    <div class="col-md-7">
                        <select name="idkecamatanx1" id="idkecamatanx1" class="w3-input w3-border" value="{{ $idkecamatan }}"></select> 
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-3">
                    ...
                    <form class="form-horizontal" id="formSimpan" nama="formSimpan" action="" method="">
                        ...
                        <div class="modal-body" id="bodyAdd" name="bodyAdd">                        
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">                                
                                    <select name="idkecamatan1" id="idkecamatan1" class="w3-input w3-border" value="{{ $idkecamatan1 }}"></select>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">                                
                                    <input name="desa1" id="desa1" class="w3-input w3-border" type="text" maxlength="50" placeholder="Desa" value="{{ old('desa1') }}" required>
                                </div>
                            </div> 
...
<script type="text/javascript">
    ...
    var listkecamatanx1Datatable;
    var listkecamatan1Datatable;
...
$(document).ready(function(){
    ...
    $('#idkabupatenx1').on('click',function(){
        setTimeout(() => {
            ...
            kirimsyarat();            	
            setTimeout(() => {
                listkecamatanx1Datatable = tampil_listkecamatanx();
                listkecamatan1Datatable = tampil_listkecamatan1();
                setTimeout(() => {
                    listkecamatanx1Datatable.ajax.url('{{route('admin.listkabupaten21')}}').load();                
                    listkecamatanx1Datatable.draw(null, false);                                                    
                    listkecamatan1Datatable.ajax.url('{{route('admin.listkabupaten21')}}').load();                
                    listkecamatan1Datatable.draw(null, false);
                }, 500);            
            }, 500);
        }, 500);        
    });

    $('#idkecamatanx1').on('click',function(){
        setTimeout(() => {
            kirimsyarat();            	
            setTimeout(() => {
                $('#idkecamatan1').val($('#idkecamatanx1').val());
    ...
    $('#idkabupaten1').on('click',function(){
        setTimeout(() => {
            kirimsyarat2();            	
            setTimeout(() => {
                listkecamatan1Datatable = tampil_listkecamatan1();
    ...
    //menampilkan combo kecamatanx
    function tampil_listkecamatanx(){				
        $.ajax({
            type: 'get',
            url   : '{{route('admin.listkecamatanx21')}}',
            
            success: function(data){				    
                $("#idkecamatanx1").html(data);
                $("#idkecamatanx1").val({{ $idkecamatan }});
            }
        })                    
    }

    //menampilkan combo kecamatan1
    function tampil_listkecamatan1(){				
        $.ajax({
            type: 'get',
            url   : '{{route('admin.listkecamatan21')}}',
            
            success: function(data){				    
                $("#idkecamatan1").html(data);
                $("#idkecamatan1").val({{ $idkecamatan1 }});
            }
        })                    
    }
    ...
    function kirimsyarat(){
        ...
        var idkecamatanx1=$('#idkecamatanx1').val();
        
        let formData = new FormData();
            formData.append('idpropinsix1', idpropinsix1);
            formData.append('idkabupatenx1', idkabupatenx1);
            formData.append('idkecamatanx1', idkecamatanx1);

        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('admin01.master.desa_kirimsyarat')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function kirimsyarat2(){
        ...
        var idkecamatan1=$('#idkecamatan1').val();
        
        let formData = new FormData();
            formData.append('idpropinsi1', idpropinsi1);
            formData.append('idkabupaten1', idkabupaten1);
            formData.append('idkecamatan1', idkecamatan1);

        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('admin01.master.desa_kirimsyarat2')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
     ...
     function tampil_data1(){
        ...
        return $('#data1').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax   : `{{route('admin01.master.desa_show')}}`,
            columns: [
    ...
    function data_simpan(){
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('admin01.master.desa_create')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function data_edit(idx){
        ...
        var id1=idx;			
            $.ajax({
		        type  : 'get',
		        url   : `{{ url('admin01/master/desaedit')}}/${id1}`,
		        async : false,
		        dataType : 'json',	
		        ...
		        success : function(data){
                    ...
                    for(i=0; i<resultData.length; i++){
                        ...
                        $('#idpropinsi1').val($('#idpropinsix1').val());
                        $('#idkabupaten1').val($('#idkabupatenx1').val());
                        $('#idkecamatan1').val(resultData[i].idkecamatan);
    ...
    function data_hapus(){			
        ...
        $.ajax({
            type  : 'get',
            url   : '{{url('admin01/master/desadestroy')}}/'+id3,
            async : false,
            dataType : 'json',					
            success : function(data){
                tampil_dataTable();
