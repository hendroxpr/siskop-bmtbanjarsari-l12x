...
@endif
                ...
                <table id="data1" class="table table-bordered table-striped table-hover" style="width: 100%">
                    <thead>
                        <tr>
							...
							<th style="width:10px">Norek Target</th>
							<th style="width:100px">Nasabah Target</th>							
                ...
                <div class="form-group">
                    <form class="form-horizontal" id="formSimpan" nama="formSimpan" action="" method="">
                        ...
                        <div class="modal-body" id="bodyAdd" name="bodyAdd">                        
                            ...
                            <div class="row">
                                <div class="col-md-12 mt-0 mb-2 text-center">
                                    <hr>
                                    <h4>Target :</h4>                        
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8 input-group">
                                    <input name="cariid1" id="cariid1" class="form-control w3-input w3-border rounded-0" type="search" value="{{ session('cariid1') }}" placeholder="Search" >                        
                                    <div class="input-group-append">
                                      <button id="btn_cariid1" name="btn_cariid1" type="button" style="border-radius:0px; border:none;"><i style="font-size:24" class="fas">&#xf002;</i></button>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-4 mt-2 text-right">
                                    <h6>Nama Nasabah</h6> 
                                    <input name="idnasabah1" id="idnasabah1" type="hidden" value="{{ session('idnasabah1') }}">
                                </div>
                                <div class="col-md-8">
                                    <input name="namalengkap1" id="namalengkap1" class="w3-input w3-border" type="text" placeholder="Nama Nasabah" value="{{ session('namalengkap1') }}" readonly>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-8">
                                    <input name="ecard1" id="ecard1" class="w3-input w3-border" type="text" placeholder="Nomor Rekening" value="{{ session('ecard1') }}" readonly>                         
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-4 mt-2 text-right">
                                    <h6>Produk</h6> 
                                    <input name="idproduk1" id="idproduk1" type="hidden" value="{{ session('idproduk1') }}">
                                </div>
                                <div class="col-md-8">
                                    <input name="produk1" id="produk1" class="w3-input w3-border" type="text" placeholder="Produk" value="{{ session('produk1') }}" readonly>
                                </div>
                            </div>
	...
	<div class="modal fade" id="ModalCariID"  data-backdrop="static">
		<div class="modal-dialog modal-lg">  <!-- modal-(sm, lg, xl) ukuran lebar modal -->
			<div class="modal-content bg-info w3-animate-zoom">
				...
				<div class="modal-header">
						<h3 class="modal-title"><i style="font-size:18" class="fas">&#xf002;</i><b> Cari Data Nasabah</b></h3>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span></button>
    ...
    <!-- ModalCariID1 modal fade-->
	<div class="modal fade" id="ModalCariID1"  data-backdrop="static">
		<div class="modal-dialog modal-lg">  <!-- modal-(sm, lg, xl) ukuran lebar modal -->
			<div class="modal-content bg-info w3-animate-zoom">
				...
				<form class="form-horizontal">
					...
					<div class="modal-body">
						...
						<div class="row">
							<div id="reload" class="table-responsive">
								<table id="carisiswa1" width="100%" class="table table-bordered table-striped table-hover">
									<thead>
									<tr>
										<th width="10px">#</th>
										<th width="50px">No.Rekening</th>																									
								...
								<tfoot id="show_footercarisiswa1">
									
								</tfoot>
								<tbody id="show_datacarisiswa1">
...
$(document).ready(function(){
    ...
    data1Datatable = tampil_data1();    
     function tampil_data1(){
        ...
        return $('#data1').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax   : `{{route('sibm01.transaksi.tfkeluar_show')}}`,
            columns: [
                ...
                { data: 'norekt', name: 'norekt'},
                { data: 'namalengkapt', name: 'namalengkapt'},
     ...
     function tampil_data_carisiswa(){
        ...
        return $('#carisiswa').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax   : `{{route('sibm01.transaksi.tfkeluar_shownasabah')}}`,
            columns: [
                // { data: 'no', name:'id', render: function (data, type, row, meta) {
                //     return meta.row + meta.settings._iDisplayStart + 1;
                // }},
     ...
     carisiswa1Datatable = tampil_data_carisiswa1();    
     function tampil_data_carisiswa1(){
        let i = 1;	
        return $('#carisiswa1').DataTable({
            responsive : true,
            retrieve: true,
            autoWidth : true,
            // buttons : [ {extend: 'colvis', postfixButtons: [ 'colvisRestore' ] }, {extend:'copy'}, {extend:'csv'}, {extend: 'pdf', orientation: 'portrait', pageSize: 'A4', title:'{{ $caption }}'}, {extend: 'excel', title: '{{ $caption }}'}, {extend:'print', orientation: 'portrait', pageSize: 'A4', title: '{{ $caption }}'}, ],        
            ...
            processing: true,
            serverSide: true,
            ajax   : `{{route('sibm01.transaksi.tfkeluar_shownasabah1')}}`,
            columns: [
                // { data: 'no', name:'id', render: function (data, type, row, meta) {
                //     return meta.row + meta.settings._iDisplayStart + 1;
                // }},
    ...
    function cariid(cari){
        ...
        $.ajax({
            ...
            type   : 'post',
            url    : '{{route('sibm01.transaksi.tfkeluar_cariid')}}',
            async : false,
            dataType : 'json',
            // data : FormData,
            data : {cari1:cari1},
    ...
    function cariid1(cari){
        var cari1=cari;
        
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('sibm01.transaksi.tfkeluar_cariid1')}}',
            async : false,
            dataType : 'json',
            // data : FormData,
            data : {cari1:cari1},
    ...
    $('#btn_cariid1').on('click',function(){
        carisiswa1Datatable.draw(null, false);
        setTimeout(() => {
            $('#ModalCariID1').modal('show');
        }, 300);
    });
    ...
    $('#show_datacarisiswa1').on('click','.item_ecard',function(){
        ambilcari1(this);
    });

    $('#show_datacarisiswa1').on('click','.item_nis',function(){
        ambilcari1(this);
    });
    $('#show_datacarisiswa1').on('click','.item_nisn',function(){
        ambilcari1(this);
    });
    $('#show_datacarisiswa1').on('click','.item_nama',function(){
        ambilcari1(this);
    });
    $('#show_datacarisiswa1').on('click','.item_kelas',function(){
        ambilcari1(this);        
    });

    function ambilcari1(t){
        var data1 = $(t).attr('data1');
        var data2 = $(t).attr('data2');
        var data3 = $(t).attr('data3');
        var data4 = $(t).attr('data4');
        ...
        $('#ModalCariID1').modal('hide');
        $('#ecard1').val(data5);
        $('#namalengkap1').val(data4);
        $('#idnasabah1').val(data1);
        $('#idproduk1').val(data7);
        $('#produk1').val(data8);
    ...
    function kirimsyarat(){
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('sibm01.transaksi.tfkeluar_kirimsyarat')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function data_simpan(){
        ...
        var idnasabaht1=$('#idnasabah1').val();
        ...
        let formData = new FormData();
            ...
            formData.append('idnasabaht1', idnasabaht1);
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('sibm01.transaksi.tfkeluar_create')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function nomorbukti(){        
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('sibm01.transaksi.tfkeluar_nomorbukti')}}',
            async : false,
            dataType : 'json',
            // data : FormData,
    ...
    function nomorposting(){        
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('sibm01.transaksi.tfkeluar_nomorposting')}}',
            async : false,
            dataType : 'json',
            // data : FormData,
    ...
    function data_edit(idx){
        var id1=idx;			
            ...
            $.ajax({
                type  : 'get',
                url   : `{{ url('sibm01/transaksi/tfkeluaredit')}}/${id1}`,
                async : false,
                dataType : 'json',	
                ...
                success : function(data){
                    ...
                    for(i=0; i<resultData.length; i++){
                        ...
                        $('#cariid1').val(resultData[i].ecard);    
                        $('#idnasabah1').val(resultData[i].idnasabah);    
                        $('#namalengkap1').val(resultData[i].namalengkap);    
                        $('#ecard1').val(resultData[i].ecard);    
                        $('#idproduk1').val(resultData[i].idproduk);    
    ...
    function data_posting(){
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('sibm01.transaksi.tfkeluar_posting')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    function data_hapus(){			
        ...
        $.ajax({
            type  : 'get',
            url   : '{{url('sibm01/transaksi/tfkeluardestroy')}}/'+id3,
            async : false,
            dataType : 'json',					
            success : function(data){
                tampil_datatable();
    ...
    function swaltargettidakada(x){
        Swal.fire({
            icon: 'error',
            title: 'Oops...target has not been selected',
            text: x,
            timer:5000
        })
    }
