            ...
            <div class="col-md-6">
                <div class="row">
                    ...
                    <div class="col-md-5 input-group">
                        <input name="kodepinjamanx1" id="kodepinjamanx1" class="form-control w3-input w3-border rounded-0" type="search" value="{{ session('kodepinjamanx1') }}" placeholder="Kode Pinjaman" autocomplete="off" autofocus>                        
                        <div class="input-group-append">
                            <button id="btn_kodepinjamanx1" name="btn_kodepinjamanx1" type="button" style="border-radius:0px; border:none;"><i style="font-size:24" class="fas">&#xf002;</i></button>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-5">
                        <input name="niax1" id="niax1" class="form-control w3-input w3-border rounded-0" type="search" value="{{ session('niax1') }}" placeholder="NIA" autocomplete="off" autofocus>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-5">
                        <input name="namax1" id="namax1" class="form-control w3-input w3-border rounded-0" type="search" value="{{ session('namax1') }}" placeholder="Nama" autocomplete="off" autofocus>
                    </div>
                ...
                <table id="data1" class="table table-bordered table-striped table-hover" style="width: 100%">
                    <thead>
                        <tr>
							...
							<th style="width:50px">Kode Pinjaman</th>
   ...
   $(document).ready(function(){
    ...
    data1Datatable = tampil_data1();    
     function tampil_data1(){
        ...
        return $('#data1').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax   : `{{route('pinjaman01.laporan.detailpinjaman_show')}}`,
            columns: [
                ...
                { data: 'nia', name: 'anggota.nia', class: 'text-center' },
    ...
    function tampil_data_carianggota(){
        ...
        return $('#carianggota').DataTable({
            ...
            processing: true,  
            serverSide: true,
            ajax   : `{{route('pinjaman01.laporan.detailpinjaman_showanggota')}}`,
            columns: [
                // { data: 'no', name:'id', render: function (data, type, row, meta) {
                //     return meta.row + meta.settings._iDisplayStart + 1;
                // }},
    ...
    $('#kodepinjamanx1').keypress(function (e) { 
        $('#niax1').val('');
        $('#namax1').val('');
        if (e.which == 13) {
        var x = $('#kodepinjamanx1').val();
            cariid(x);
            setTimeout(() => {
                var x = $('#niax1').val();
                if (x==''){
                    $('#btn_kodepinjamanx1').click();  
                }else{
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            data1Datatable = tampil_data1();  
                            setTimeout(() => {
                                data1Datatable.ajax.url('{{route('pinjaman01.laporan.detailpinjaman_show')}}').load();                
    ...
    function cariid(cari){
        ...
        $.ajax({
            ...
            type   : 'post',
            url    : '{{route('pinjaman01.laporan.detailpinjaman_cariid')}}',
            async : false,
            dataType : 'json',
            // data : FormData,
            data : {cari1:cari1},
            ...
            success : function(data){ 
                ...
                var y;                
                 for(i=0; i<resultData.length; i++){                         
                        ...
                        $('#niax1').val(resultData[i].anggota.nia);
    ...
    $('#btn_kodepinjamanx1').on('click',function(){
        setTimeout(() => {
            carianggotaDatatable = tampil_data_carianggota();  
            setTimeout(() => {
                carianggotaDatatable.ajax.url('{{route('pinjaman01.laporan.detailpinjaman_showanggota')}}').load();                
    ...
    function kirimsyarat(){        
        ...
        $.ajax({
            enctype: 'multipart/form-data',
            type   : 'post',
            url    : '{{route('pinjaman01.laporan.detailpinjaman_kirimsyarat')}}',
            data: formData,
            cache: false,
            processData: false,
            contentType: false,
    ...
    $('#show_data1').on('click','.item_print',function(){ 
        ...
        kirimsyarat();
        setTimeout(() => {
            window.open('{{ route('pinjaman01.laporan.detailpinjaman_printkwitansi') }}');
        }, 100);
    });
